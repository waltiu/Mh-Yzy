<template>
  <div>
            ...
            >
              ...
              <div class="tipsitem">
                <div v-show="item.upPort!=='COM'">
                  <p>业务侧面设备名称：</p>
                  <P class="title">{{item.name||'_'}}</P>
                  <div>
                    <p>业务侧面设备ID：</p>
                    <P class="title">{{item.epId||'_'}}</P>
                  </div>
                  <div>
                    <p>业务侧面设备端口：</p>
                    <P class="title">{{item.port||'_'}}</P>
                  </div>
                  <div>
                    <p>业务使用情况：</p>
                    <P class="title">{{computedState(item.state)}}</P>
                  </div>
                </div>
              </div>
            </div>
    ...
    <el-dialog
      ...
      :modal="false"
      title="端口设置"
      v-if="portDetail.detail.upPort!=='COM'"
    >
      <detail @getPort="getPort" :portDetail="portDetail" v-if="dialogVisible"></detail>
      <span slot="footer" class="dialog-footer">
        <el-button @click="cancel">取 消</el-button>
        <el-button type="primary" @click="surePort">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    computedStyle (item) {
      ...
      if (item.state === '0') {
        return ` background: #0071bc`
      } else if (item.state === '1') {
        return ` background: #f15a24`
      } else {
