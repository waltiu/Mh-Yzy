      ...
      return [
        {
          ...
          permission: () => auth('guangluguanli.r') && auth('excelmanage.export')
        },
      ]
    },
    upload (uploadInfo) {
      return [
        {
          ...
          url: uploadInfo.url,
          permission: () => auth('guangluguanli.r') && auth('excelmanage.input')
        },
      ]
    },
    getTableInfo () {
      ...
      this.$http.get(this.$pageUrl[this.$options.name].url, {
        params: { ...this.pageStatus, ...this.$pageUrl[this.$options.name].args }
      })
        .then((res) => {
