<template>
  <div>
    ...
    <div style="display: flex;">
      ...
      >
        <div style="display: flex;">
          <el-switch @change="changeRelatedStatus" v-model="tmp"></el-switch>
          {{$t('well.wellsWithPipelines')}}
...
export default {
  ...
  methods: {
    ...
    changeRelatedStatus (e) {
      let target = e ? '/api/ldsysPoints/search/findUnReferenceByName' : '/api/ldsysPoints/search/findByNameLikeIgnoreCaseAndType'
      this.getTableInfo(target)
    },
    getTableInfo (url = this.$pageUrl[this.$options.name].url) {
      // 重置状态
      this.rowData = [0]
      this.$http.get(url, {
