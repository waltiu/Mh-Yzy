  ...
  <div>
    <el-card class="box-card">
      ...
      <div class="text item" v-for="(a, index) in munitBindData" :key="index">
        ...
        <div>{{$t('bindPort.lightBoxPort')}}: {{ $portNumToShow({port: a.minPort, dist: 12, frame: odfInfo.rectangleRowNum}) }} {{$t('bindPort.to')}} {{ $portNumToShow({port: a.maxPort, dist: 12, frame: odfInfo.rectangleRowNum}) }}</div>
        <hr />
      </div>
    </el-card>
...
export default {
  ...
  computed: {
    munitBindData () {
      ...
      // let temp = new Map()
      // let group = []
      if (_.isEmpty(this.odfInfo.useInfos)) return
