<template>
  <div>
                ...
                <div style=" height:20px;width:100%; display:flex; align-items:flex-start">
                  ...
                  >
                    <div
                      :style="portStyle(c)"
                      :title="getTitle(c)"
                      @click="selectPort(index + 1, indexB + 1, indexC + 1, c)"
                    ></div>
                  </span>
                </div>
              </div>
    ...
    <change-use-status :info="changeStatus"></change-use-status>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    portStyle (info) {
      ...
      return `cursor:pointer;border-radius:50% ;  background:${color}; width:17px;height: 17px`
    },
    getTitle (info) {
      let title = '暂无'
